...
- (void) prepareForSegue:(UIStoryboardSegue*)segue sender:(id)sender {
    ...
    if (kDebugLevel > 1)
        NSLog(@"AddProviderViewController:prepareForSegue: our identity: %s, deposit: %s, public-key: %s, and provider's identity: %s.", [_our_data.identity cStringUsingEncoding: [NSString defaultCStringEncoding]], [[PersonalDataController absoluteStringDeposit:_our_data.deposit] cStringUsingEncoding:[NSString defaultCStringEncoding]], [[_our_data.getPublicKey base64EncodedString] cStringUsingEncoding:[NSString defaultCStringEncoding]], [_provider.identity cStringUsingEncoding: [NSString defaultCStringEncoding]]);
